...
void eat(int position)
{
	...
	if (state[curr] == 1 && state[l_fork] != 0 && state[r_fork] != 0 && bowl(curr) == 1) 
	{
		
		state[curr] = 0;
		sleep(3);
		
		printf("Philosopher %d takes fork (%d,%d) and bowl %d \n",curr+1, l_fork+1, curr+1, cur_pos(curr));
...
void take_leave(int position)
{
	...
	printf("Philosopher %d putting fork (%d,%d) and bowl %d down\n",position+1,l_fork+1, position+1,cur_pos(position));
